...
if __name__ == "__main__":
    ...
    exp_path = f'/content/gdrive/MyDrive/cv/Facial-Landmarks-Detection-Pytorch/experiments/experiment5'

    # Load parameters from experiment config
    variables = {}
    # exec(open(exp_path + '/config.py').read(), variables)
    exec(open('/content/gdrive/MyDrive/cv/Facial-Landmarks-Detection-Pytorch/experiments/experiment5/config.py').read(), variables)
    ...
    df = pd.read_csv('/content/gdrive/MyDrive/cv/emotion_data_one.csv')
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        if epoch % 10 == 0:
            torch.save(MODEL.state_dict(), f'/content/gdrive/MyDrive/cv/Facial-Landmarks-Detection-Pytorch/experiments/experiment5/model_epoch_{epoch}.pth')
...
plt.legend()
plt.savefig(f'/content/gdrive/MyDrive/cv/Facial-Landmarks-Detection-Pytorch/experiments/experiment5/train_val_loss.png')

# Saving predictions
with open(f'/content/gdrive/MyDrive/cv/Facial-Landmarks-Detection-Pytorch/experiments/experiment5/val_preds.pkl', 'wb') as f:
    pickle.dump(val_preds, f)

with open(f'/content/gdrive/MyDrive/cv/Facial-Landmarks-Detection-Pytorch/experiments/experiment5/val_labels.pkl', 'wb') as f:
    pickle.dump(val_labels, f)

# Save classification report
cr = classification_report(val_labels, val_preds, output_dict=True)
df_cr = pd.DataFrame(cr).transpose()
df_cr.to_csv(f'/content/gdrive/MyDrive/cv/Facial-Landmarks-Detection-Pytorch/experiments/experiment5/classification_report.csv')

with open(f'/content/gdrive/MyDrive/cv/Facial-Landmarks-Detection-Pytorch/experiments/experiment5/classification_report.tex', 'w') as f:
    f.write(df_cr.to_latex())

with open(f'/content/gdrive/MyDrive/cv/Facial-Landmarks-Detection-Pytorch/experiments/experiment5/classification_report.txt', 'w') as f:
    f.write(df_cr.to_latex())